...
def main(argv):
  ...
  while 1:
    ...
    marker_ids = build_markers(p.getNumJoints(robot))
    ...
    for i in range(p.getNumJoints(robot)):
      joint_pos.append(p.getJointInfo(robot, i)[14])
      print(p.getJointInfo(robot,i)[0:2], "\n")
      print(p.getJointInfo(robot,i)[8:10], "\n")

    joint_pos = np.array(joint_pos)
                       
 

    set_maker_pos(joint_pos, marker_ids)
