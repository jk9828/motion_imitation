...
"""
REF_SHOUL_KNEE_JOINT_ID = [3, 10, 7, 14 ]
REF_CLE_JOINT_IDS = [1, 5]

#REF_HIP_JOINT_IDS = [6, 16, 11, 20]
REF_HIP_JOINT_IDS = [2, 9, 6, 13]

#REF_TOE_JOINT_IDS = [10, 19, 15, 23]
REF_TOE_JOINT_IDS = [4, 12, 8, 16]
...
mocap_motions = [
  ["Human motion", "data/bone_animation_dataaa.txt", 1,150],
]
...
def build_markers(num_markers): # func buil_markers(num_markers) : 상체/하체/발의 마커를 생성 및 색을 입히는 함수 -> 마커를 리턴
  ...
  for i in range(num_markers):
    if (i == check):
      col = [1, 1, 1, 1]
    elif (i == REF_NECK_JOINT_ID) or (i == REF_PELVIS_JOINT_ID)\
